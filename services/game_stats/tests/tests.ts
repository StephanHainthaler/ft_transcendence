...
import { recordMatch, getUserStats, getLeaderboard, updateStatsForUser, getUserMatchHistory } from '../src/logic';
import { eq } from '@server/orm';
import { UserStats, MatchSubmissionData } from '@shared/game_stats';
...
describe('Game Stats Logic Tests', () => {
	...
	test('should correctly update rank/streak after first match (A wins)', () => {
		...
		const statsC = getUserStats(USER_C) as UserStats;
		...
		// Match history A
			assert.strictEqual(history.length, 1, 'History count should be 1');
            //assert.strictEqual(history[0].winner_id, USER_A, 'History should record correct winner');
        });
	...
	test('4. Should not allow rank to drop below 0', () => {
		...
		const statsC = getUserStats(USER_C) as UserStats;
		assert.strictEqual(statsC.rank, 0, 'Rank should not be negative');
	});
});
...
/* describe('Read Operations (History, Leaderboard, Single Match)', () => {
	...
	beforeEach(() => {
        ...
        test('5. getUserMatchHistory should correctly filter and paginate', () => {
            ...
            for (let i = 0; i < 5; i++) {
                recordMatch({MATCH_DATA_B_WINS, winner_id: USER_C, player_one_id: USER_C });
            }
