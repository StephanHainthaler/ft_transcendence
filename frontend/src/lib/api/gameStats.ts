...
async function fetchUserStats(userId: number): Promise<UserStats | null> {
    try {
        const req = new Request(`${import.meta.env.VITE_API_URL}/stats/v1/user/${userId}`, {
            method: "GET",
        });
...
async function fetchMatchHistory(userId: number, page: number = 1): Promise<MatchHistoryEntry[]> {
    try {
        const req = new Request(`${import.meta.env.VITE_API_URL}/stats/v1/history/${userId}?page=${page}`, {
            method: "GET",
        });
        const response = await request(req);
        return response || [];
...
async function fetchLeaderboard(page: number = 1): Promise<UserStats[] | []> {
    try {
        const req = new Request(`${import.meta.env.VITE_API_URL}/stats/v1/leaderboard?page=${page}`, {
            method: "GET",
        });
        const response = await request(req);
        return response || [];
...
const sendMatchResults = async (matchData: MatchSubmissionData) => {
    try {
        const req = new Request(`${import.meta.env.VITE_API_URL}/stats/v1/match`, {
            method: 'POST',
            headers: {
