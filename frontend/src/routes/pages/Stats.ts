...
const StatsFormDiv = (): VNode =>
{
    ...
    return div({ id: 'stats-content', class: 'card' },
        h1({ class: 'title' }, 'Player statistic'),
        div({ class: 'rank-box' }, 
        span({}, 'Ваш ранг: '),
...
const MatchHistoryTable = (matches: MatchHistoryEntry[]): VNode => {
    return div({ class: 'mt-6' },
        h2({ class: 'text-xl font-bold mb-2' }, 'Історія матчів'),
        div({ class: 'overflow-x-auto' }, 
            el('table', { class: 'w-full text-left border-collapse' },
                el('thead', { class: 'bg-teal-700 text-white' },
                    el('tr', {},
                        el('th', { class: 'p-2' }, 'Супротивник'),
                        el('th', { class: 'p-2' }, 'Рахунок'),
                        el('th', { class: 'p-2' }, 'Результат')
                    )
                ),
                el('tbody', {},
                    ...matches.map(m => {
                        ...
                        return el('tr', { class: 'border-b border-gray-200' },
                            el('td', { class: 'p-2' }, `Гравець #${m.opponent_id}`), // Тут можна додати ім'я, якщо воно є
